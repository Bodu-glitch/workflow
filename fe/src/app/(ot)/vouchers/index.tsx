...
export default function OTVouchersScreen() {
  ...
  const toggleMutation = useMutation({
    mutationFn: (voucher: Voucher) => vouchersApi.update(voucher.id, { is_active: !voucher.is_active }),
    ...
    <View className="flex-1 bg-surface">
      <View className="glass-effect px-5 pt-14 pb-4">
        <View className="flex-row items-center justify-between">
          ...
          <Pressable
            onPress={() => router.push('/(ot)/vouchers/create')}
      ...
      >
        {vouchers.length === 0 ? (
          ...
          vouchers.map(v => (
            <VoucherCard key={v.id} item={v} onToggle={() => toggleMutation.mutate(v)} />
          ))
        )}
        <View className="h-8" />
      </ScrollView>
