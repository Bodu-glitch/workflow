...
export default function OTRequestDetailScreen() {
  ...
  const assignMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['service-request-ot', id] });
    },
    onError: (e: any) => {
  ...
  return (
    <View className="flex-1 bg-surface">
      ...
      >
        ...
        {/* Payment details */}
        {(req.collected_amount != null || req.final_amount != null) && (
          <View className="bg-surface-container-lowest rounded-xl p-5 mb-4">
            <Text className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant mb-3">Thanh toán</Text>
            {req.collected_amount != null && (
              <View className="flex-row items-center justify-between mb-2">
                <Text className="text-sm text-on-surface-variant">Đã thu</Text>
                <Text className="text-sm font-bold" style={{ color: '#10B981' }}>{req.collected_amount.toLocaleString('vi-VN')}₫</Text>
              </View>
            )}
            {req.final_amount != null && (
              <View className="flex-row items-center justify-between">
                <Text className="text-sm text-on-surface-variant">Tổng tiền</Text>
                <Text className="text-sm font-bold text-on-surface">{req.final_amount.toLocaleString('vi-VN')}₫</Text>
              </View>
            )}
          </View>
        )}
