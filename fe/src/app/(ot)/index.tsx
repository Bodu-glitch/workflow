...
function DonutRing({ percentage, size = 90, strokeWidth = 11 }: { percentage: number; size?: number; strokeWidth?: number }) {
  ...
  const angle1 = r1 * 180 - 135;
  const angle2 = r2 * 180 - 135;

  return (
    <View style={{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }}>
      <View style={{ position: 'absolute', width: size, height: size, borderRadius: half, borderWidth: strokeWidth, borderColor: '#dce9ff' }} />
      <View style={{ position: 'absolute', width: half, height: size, left: half, overflow: 'hidden' }}>
        <View style={{
          ...
          borderRadius: half, borderWidth: strokeWidth, borderColor: '#1E40AF',
