        ...
        <Section title="Assigned Staff">
          ...
          {canModify && unassignedStaff.length > 0 && (
            <View className="mt-3 pt-3" style={{ borderTopWidth: 1, borderTopColor: 'rgba(195,198,214,0.3)' }}>
              <Text className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant mb-2">Add Staff</Text>
              {unassignedStaff.map((s) => (
                <Pressable key={s.id} onPress={() => assignMutation.mutate(s.id)} className="py-2 active:opacity-60">
