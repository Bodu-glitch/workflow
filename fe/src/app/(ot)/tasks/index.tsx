...
function TaskCard({ task }: { task: Task }) {
  const pillColor = task.priority
    ? PRIORITY_PILL_COLOR[task.priority]
    : "bg-outline";
  return (
    <Pressable
      onPress={() =>
        router.push({ pathname: "/(ot)/tasks/[id]", params: { id: task.id } })
      }
    ...
    >
      ...
      <View className="flex-row items-start justify-between mb-3">
        <Text
          className="text-base font-bold text-on-surface flex-1 mr-3"
          numberOfLines={2}
        >
          {task.title}
        </Text>
...
export default function OTTaskListScreen() {
  ...
  const serviceOptions = [
    { label: "All", value: undefined },
    ...(filterOptionsData?.service_types ?? []).map((s) => ({
      label: s,
      value: s,
    })),
  ];
