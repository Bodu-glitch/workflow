...
function MyScheduleTab({ userId }: { userId?: string }) {
  ...
  const assignments: MyShiftAssignment[] = data?.data ?? [];
      ...
      <View className="bg-white border-b border-surface-variant">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, gap: 8, paddingBottom: 12 }}>
          {weekDates.map((d, i) => {
            ...
            return (
              <View key={i} style={{ alignItems: 'center', paddingVertical: 6, paddingHorizontal: 10, borderRadius: 12, backgroundColor: isToday ? '#1E40AF' : '#f3f4f6', minWidth: 48 }}>
                <Text style={{ fontSize: 10, fontWeight: '600', color: isToday ? '#bfdbfe' : '#9ca3af', textTransform: 'uppercase' }}>{DOW_SHORT[i]}</Text>
                <Text style={{ fontSize: 16, fontWeight: '800', color: isToday ? '#fff' : '#374151' }}>{d.getDate()}</Text>
                {hasShift && <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: isToday ? '#93c5fd' : '#1E40AF', marginTop: 2 }} />}
              </View>
            );
          })}
        </ScrollView>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        {!isLoading && assignments.length === 0 && (
          <View className="items-center py-20">
            ...
            <Text className="text-on-surface-variant text-sm text-center">Bạn chưa được phân ca\ntrong tuần này</Text>
          </View>
        )}
              ...
              {dayShifts.map((a) => {
                ...
                return (
                  <View
                    ...
                    style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.07)', borderLeftWidth: 4, borderLeftColor: shift.color }}
                  >
...
function LeaveRequestsTab() {
  ...
  const createMut = useMutation({
    mutationFn: () =>
      scheduleApi.createLeaveRequest({ type: leaveType, start_date: startDate, end_date: endDate, reason: reason || undefined }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['my-leave-requests'] });
      showToast('Đơn nghỉ phép đang chờ duyệt', 'success', 'Đã gửi đơn');
      setModal(false);
      setStartDate('');
      setEndDate('');
      setReason('');
    ...
    <>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        {requests.map((req) => {
          ...
          return (
            <View key={req.id} className="bg-white rounded-2xl mb-3 p-4" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.07)' }}>
              ...
              {req.reason ? <Text className="text-sm text-on-surface mt-1">{req.reason}</Text> : null}
              {req.reject_reason ? (
      ...
      <Modal visible={modal} transparent animationType="slide">
        ...
        <View className="bg-white rounded-t-3xl px-6 pt-6 pb-10">
          ...
          <Text className="text-xs font-bold text-on-surface-variant mb-1.5 uppercase tracking-wide">Lý do (tuỳ chọn)</Text>
          <TextInput
            value={reason}
