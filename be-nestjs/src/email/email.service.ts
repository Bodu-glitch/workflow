...
export class EmailService {
  ...
  constructor(private config: ConfigService) {
    ...
    this.frontendUrl = this.config.get<string>('APP_FRONTEND_URL', 'http://localhost:8081');

    const smtpHost = this.config.get<string>('SMTP_HOST');
    if (smtpHost) {
      this.smtp = nodemailer.createTransport({
        host: smtpHost,
        port: this.config.get<number>('SMTP_PORT', 54325),
        secure: false,
        ignoreTLS: true,
      });
    }
...
<p>Lien ket co hieu luc trong 24 gio.</p>
  ...
  async sendInvitationAcceptEmail(email: string, token: string): Promise<void> {
    ...
    try {
      await this.send(
        email,
        'Loi moi tham gia workspace moi',
        `<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body>
